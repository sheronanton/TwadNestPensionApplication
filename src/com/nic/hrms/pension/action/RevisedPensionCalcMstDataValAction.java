...
public class RevisedPensionCalcMstDataValAction extends BaseAction {

	/**
	 * 
	 */
	private static final long serialVersionUID = 2580606530771088937L;
	private RevisedPensionCalculationMstDataValService revisedpensioncalculationvalservice;
	private PensionApplicationCalcSearchModel penAppHOCalcComm;
	private RevisedPensionCalculationDao revPensionValCalculation;
	...
	private List<RevisedPensionCalcOthDeptServiceDao> revvalotherdao;
	...
	public String loadSettingData()
	{
		...
		try
		{
			...
			mysettingList=revisedpensioncalculationvalservice.revPenCalcSettingData();
			myavgsettingList=revisedpensioncalculationvalservice.revPenCalcAvgSettingData();			
			  int dapercentage=0;
	...
	public String loadRevisedPenCalcChangeMstData()
	{
		int loginEmpId=0;
		int curloginEmpId=0;
		...
		try
		{
			...
			loginEmpId=revPensionValCalculation.getEmpNo();
			
					
			myList=revisedpensioncalculationvalservice.revisedPenCalculationMstChangeData(loginEmpId,curloginEmpId);
			int dapercentage=0;
				...
				for(Object[] tl: myList)
				 {
					...
					if(temp[16]!=null)
					{
						...
						dapercentage=revisedpensioncalculationvalservice.revPenCalcdaSettingData(dor);
					}
					else
					{
	...
	public String loadRevPenChangeData()
	{
		int loginEmpId=0;
		int curloginEmpId=0;
		...
		try
		{
			...
			myCheckvalList=revisedpensioncalculationvalservice.revisedPenCalcChangeData(loginEmpId,curloginEmpId);
	...
	public String loadPenAppHOForm1OldDeptValData()
	{
		...
		PrintWriter out = getResponse().getWriter();
		empId=revPensionValCalculation.getEmpNo();
		...
		List<RevisedPensionCalcOthDeptService> othdeptserli=revisedpensioncalculationvalservice.loadHOOfficeForm1OldOtherDepServiceVal(empId);
		for(RevisedPensionCalcOthDeptService othdeptsertmp:othdeptserli)
		{
	...
	public String loadRevPenCalculationChangeData()
	{
		int loginEmpId=0;
		...
		try
		{
			...
			UserProfile empProfile=(UserProfile)session.getAttribute("UserProfile");
			loginEmpId=revPensionValCalculation.getEmpNo();
			myOldList=revisedpensioncalculationvalservice.revisedPenCalculationChangeData(loginEmpId);
	...
	public String loadRevPenCalculationChangeAEData()
	{
	...
	try
		...
		List<RevisedPensionCalculationAverageEmolumentsDao> aeli=revisedpensioncalculationvalservice.loadRevisedChangeAe(empId);
	...
	public String loadRevPenCalcComData()
	{
		...
		try
		{
			PrintWriter out = getResponse().getWriter();	
			HttpSession session=getRequest().getSession();
			int age=revPensionValCalculation.getAge();
			
			
			Double commvalue=revisedpensioncalculationvalservice.revPenCalcComSettingData(age);
	...
	public String loadRevPenCalcEmpStatus()
	{
		...
		try
		{
			PrintWriter out = getResponse().getWriter();	
			HttpSession session=getRequest().getSession();
			int empId=revPensionValCalculation.getEmpNo();
			
			myempstatusList=revisedpensioncalculationvalservice.revPenCalcEmpStatusData(empId);
	...
	public String addRevisedPensionValCalculation()
	{
		...
		{ 		
			...
			Date deatdate=revPensionValCalculation.getDeath_date1();			
			...
			if(basic_pay != null){
				...
				revisedpensioncalculationvalservice.deleteRevisedAvgemoluments(revPensionValCalculation.getEmpNo());
			    ...
			    for(int i=0; i<basic_pay.size(); i++)
			    {
			    	...
			    	form1aemultidata.setEmpNo(revPensionValCalculation.getEmpNo());
			    ...
			    revisedpensioncalculationvalservice.saveRevisedPenValAverageEmoluments(revPenaemultidatlist);
			}
			
			}*/
			revisedpensioncalculationvalservice.deleteHOForm1OthdeptdetailsVal(revPensionValCalculation.getEmpNo());			
			RevisedPensionCalcOthDeptServiceDao otherdeptvalservice=null;
			revvalotherdao=new ArrayList<RevisedPensionCalcOthDeptServiceDao>();			
			if(revPensionValCalculation.getOtherdeparmentserviceFlag().equals("Yes"))
			{
				...
				{
					otherdeptvalservice=new RevisedPensionCalcOthDeptServiceDao();
					compositepk = new RevisedPensionCalcOthDeptServiceValDaoPK();
					compositepk.setEmpNo(revPensionValCalculation.getEmpNo());
					compositepk.setFromdate(othdeptfromdate[i]);
					otherdeptvalservice.setCompositepk(compositepk);
					otherdeptvalservice.setTodate(othdepttodate[i]);
					otherdeptvalservice.setOthServiceYear(otdepyear[i]);
					otherdeptvalservice.setOthServiceMonth(otdepmonth[i]);
					otherdeptvalservice.setOtherServiceDay(othdeptday[i]);
					otherdeptvalservice.setDepartmentName(Departmentname[i]);
					otherdeptvalservice.setRemarks(Remarks[i]);
					otherdeptvalservice.setUpdatedUser(updatedId);
					otherdeptvalservice.setUpdateDate(new java.sql.Timestamp(new java.util.Date().getTime()));
					otherdeptvalservice.setUnlockedBy(updatedId);
					otherdeptvalservice.setUnlockedDate(new java.sql.Timestamp(new java.util.Date().getTime()));
					
					revvalotherdao.add(otherdeptvalservice);
			}
				revisedpensioncalculationvalservice.saveAddHOOtherDepartmentServiceVal(revvalotherdao);
			...
			*/
				...
				age=revPensionValCalculation.getAge();
				
				Integer commutedamount=revPensionValCalculation.getCommutedamount();
				float commfloatval=Float.parseFloat(revisedpensioncalculationvalservice.revPenCalcComSettingData(age)+"");					
				revPensionValCalculation.setCommuted_val(commfloatval);
				float commutedpensionamount=(commutedamount)*12*(commfloatval);
				Integer commutedpenamount=(int)Math.round(commutedpensionamount);
				revPensionValCalculation.setTotcommutedamount(commutedpenamount);
				...
				Date vrsdate=revPensionValCalculation.getVrs_date1();
				Date retiredate=revPensionValCalculation.getDar1();
				Date dedate=revPensionValCalculation.getDeath_date1();
				String dor="";
				if(vrsdate!=null)
				{
					DateFormat formatter= new SimpleDateFormat("dd/MM/yyyy");
				...
				int dapercentage=revisedpensioncalculationvalservice.revPenCalcdaSettingData(dor);
				
				
				revPensionValCalculation.setDa_percentage(dapercentage);
				  ...
				  String basic_include=revPensionValCalculation.getAvg_include_basic();
				  String basic_da=revPensionValCalculation.getAvg_da_basic();
				  			  
				  String grade_include=revPensionValCalculation.getAvg_include_grade();
				  String grade_da=revPensionValCalculation.getAvg_da_grade();
				 			  
				  String special_include=revPensionValCalculation.getAvg_include_special();
				  String special_da=revPensionValCalculation.getAvg_da_special();
				  
				  String other1_include=revPensionValCalculation.getAvg_include_other1();
				  String other1_da=revPensionValCalculation.getAvg_da_other1();
				  
				  String other2_include=revPensionValCalculation.getAvg_include_other2();
				  String other2_da=revPensionValCalculation.getAvg_da_other2();
				  
				  String other3_include=revPensionValCalculation.getAvg_include_other3();
				  String other3_da=revPensionValCalculation.getAvg_da_other3();
				  ...
				  finaltotalppamount=revPensionValCalculation.getFinaltotppamount();
				  if(revPensionValCalculation.getLastbasic()==null)
				  {
					  lastbasic=0;
				  }
				  else
				  {
				  lastbasic=revPensionValCalculation.getLastbasic();
				  }
				  if(revPensionValCalculation.getLastgrade()==null)
				  {
					  lastgrade=0;
				  }
				  else
				  {
					  lastgrade=revPensionValCalculation.getLastgrade();
				  }
				  if(revPensionValCalculation.getLastspecial()==null)
				  {
					  lastspecial=0;
				  }
				  else
				  {
					  lastspecial=revPensionValCalculation.getLastspecial();
				  }
				  if(revPensionValCalculation.getLastother1()==null)
				  {
					  lastother1=0;
				  }
				  else
				  {
					  lastother1=revPensionValCalculation.getLastother1();
				  }
				  if(revPensionValCalculation.getLastother2()==null)
				  {
					  lastother2=0;
				  }
				  else
				  {
					  lastother2=revPensionValCalculation.getLastother2();
				  }
				  if(revPensionValCalculation.getLastother3()==null)
				  {
					  lastother3=0;
				  }
				  else
				  {
					  lastother3=revPensionValCalculation.getLastother3();
				  }
				  ...
				  maxdcrgamount=revPensionValCalculation.getMax_dcrg_amt();
				  dcrghalfyear=revPensionValCalculation.getNohalfyeardcrg();
				 ...
				 /* if((grade_include.equalsIgnoreCase("Y"))&&(grade_da.equalsIgnoreCase("Y")))
				  ...
				  String scale=revPensionValCalculation.getScaleOfPay();
				  System.out.println("scale::"+scale);
				  
				  revPensionValCalculation.setScaleOfPay(scale);
				  
				  
				  if(!(revPensionValCalculation.getDeath_date1()+"").equalsIgnoreCase("null"))
				  {
						Integer nqs_year=0;
						Integer multvar_dcrg=1;
						float lpd=lasttotal;
						if(!(revPensionValCalculation.getNqs_year()+"").equalsIgnoreCase("null"))
						{
							  nqs_year=revPensionValCalculation.getNqs_year();
						}
						if(nqs_year<1)
						{
							multvar_dcrg=2;
				  ...
				  revPensionValCalculation.setDamount((int)Math.round(damount));				
				  revPensionValCalculation.setDcrgamount((int)Math.round(dcrgpensionamuont));
									
				  revPensionValCalculation.setProcessStatus("VALIDATE");
				  revPensionValCalculation.setUpdatedUser(updatedId);
				  revPensionValCalculation.setUpdateDate(new java.sql.Timestamp(new java.util.Date().getTime()));
				  revPensionValCalculation.setUnlockedBy(updatedId);
				  revPensionValCalculation.setUnlockedDate(new java.sql.Timestamp(new java.util.Date().getTime()));
				
			
			
				  if(revisedpensioncalculationvalservice.saveRevisedPensionValCalculation(revPensionValCalculation))
				  {
					  System.out.println("Change Table Updated");
					  reportEmpId=revPensionValCalculation.getEmpNo();		
					  revisedpensioncalculationvalservice.moveOldPensionDetailsIntoHistory(reportEmpId);
					  
				  }
				  
				reportEmpId=revPensionValCalculation.getEmpNo();
				Date deathdate=revPensionValCalculation.getDeath_date1();
				Date vdate=revPensionValCalculation.getVrs_date1();
				Date retdate=revPensionValCalculation.getDar1();
	...
	public RevisedPensionCalculationMstDataValService getRevisedpensioncalculationvalservice() {
		return revisedpensioncalculationvalservice;
	}
	public void setRevisedpensioncalculationvalservice(
			RevisedPensionCalculationMstDataValService revisedpensioncalculationvalservice) {
		this.revisedpensioncalculationvalservice = revisedpensioncalculationvalservice;
	}	
	public RevisedPensionCalculationDao getRevPensionValCalculation() {
		return revPensionValCalculation;
	}
	public void setRevPensionValCalculation(
			RevisedPensionCalculationDao revPensionValCalculation) {
		this.revPensionValCalculation = revPensionValCalculation;
	}
	...
	public List<RevisedPensionCalcOthDeptServiceDao> getRevvalotherdao() {
		return revvalotherdao;
	}
	...
	public void setRevvalotherdao(
			List<RevisedPensionCalcOthDeptServiceDao> revvalotherdao) {
		this.revvalotherdao = revvalotherdao;
	}
