...
public class RevisedPensionCalculationMstDataDaoImpl implements RevisedPensionCalculationMstDataService{
	...
	public List<Object[]> revisedPenCalculationMstData(int loginEmpId, int curloginEmpId) 
	{
		...
		{			
			...
			*/
			...
			String strQuery="select form1_det.emp_id," +
					...
					"from hrm_pen_app_ho_mst_form1_det form1_det " +
	...
	public List<Object[]> revisedPenCalcCheckValData(int loginEmpId, int curloginEmpId) 
	{
		List<Object[]> list=null;		
		Session session=SessionFactoryUtils.doGetSession(sessionFactory, true);
		Transaction tx=null;
	...
	public List<Object[]> revisedPenCalculationOldData(int loginEmpId) 
	{
		...
		{			
			...
			String strOldQuery="select EMP_ID," +
             ...
             System.out.println("strOldQuery==="+strOldQuery);
	...
	@SuppressWarnings("unchecked")
	public List<RevisedPensionCalculationAverageEmolumentsDao> loadRevisedOldAe(int empid) 
	{
	...
	public boolean saveRevisedPensionCalculation(RevisedPensionCalculationDao revisedpensionCalc)
	{		
		...
		{							
			trans = session.beginTransaction();					
			session.saveOrUpdate(revisedpensionCalc);
			trans.commit();
		} 	
	...
	public boolean saveRevisedPenAverageEmoluments(List<RevisedPensionCalculationAverageEmolumentsDao> revPenaverageemoluments) {
	...
	try{
		trans=session.beginTransaction();
		System.out.println("***********-------Passed emp_id --------*********"+emp_id);
	...
	@SuppressWarnings("unchecked")
	public List<RevisedPensionCalcOthDeptServiceDao> loadHOOfficeForm1OldOtherDepServiceVal(
			Integer empId) {
		// TODO Auto-generated method stub
		 List<RevisedPensionCalcOthDeptServiceDao> hoolddeptser=null;
		Session session = SessionFactoryUtils.doGetSession(sessionFactory, true);
		Transaction trans = null;
		try 
		{	
			trans = session.beginTransaction();				
			Query qry=session.createQuery("from RevisedPensionCalcOthDeptServiceDao where compositepk.empNo="+empId+" order by compositepk.fromdate1");				
	...
	public boolean deleteothertdeptmst(int empNo) {
		...
		try{
			trans=session.beginTransaction();
			Query query = session.createSQLQuery("delete from HRM_PEN_APP_HO_CO_OTH_DEPT_SER where EMPLOYEEID="+empNo);		
			query.executeUpdate();
			trans.commit();
	...
	@Override
	public boolean saveothherdeptservice(
		...
		try {	
			...
			for(RevisedPensionCalcOthDeptService dao: revOtherservicelist)
			{
