...
public class PensionApplicationForm1CalcMstDataDaoImpl implements PensionApplicationForm1CalcMstDataService{
	...
	public List<Object[]> penAppForm1MstData(int loginEmpId, int curloginEmpId) 
	{
		...
		try		
		{
			...
			System.out.println("loginEmpId::"+loginEmpId+" "+"curloginEmpId::"+curloginEmpId);
			String strQuery="SELECT emp.EMPLOYEE_ID, " +
					"emp.EMPLOYEE_NAME, " +
					"emp.EMPLOYEE_INITIAL,  " +
					"emp.GPF_NO," +
	...
	public List<Object[]> penAppForm1MstCheckValData(int loginEmpId, int curloginEmpId) 
	{
		List<Object[]> list=null;
	...
	public List<Object[]> penAppForm1CalcOldMstData(int loginEmpId) 
	{
		...
		try		
		{
			...
			String strOldQuery="select EMP_ID," +
					...
					"DEATH_DATE," +
					"OTH_DEPT_SERV_FLAG , " +
					"LEVEL_OF_PAY  from HRM_PEN_APP_FORM1_DETAILS where EMP_ID="+loginEmpId;			
					
             System.out.println(strOldQuery);
	...
	public List<PensionApplicationForm1AverageEmolumentsDao> loadForm1OldAe(int empid) {
	...
	public List<PensionApplicationForm1OthDeptServiceDao> loadForm1OldOtherDepService(int empid) {
		// TODO Auto-generated method stub
	...
	public List<PensionApplicationForm1RecoveriesDao> loadForm1OldRecoveries(int empid) {
		// TODO Auto-generated method stub
	...
	public List<Object[]> penAppForm1CalcSettingData() 
	{
	...
	public List<Object[]> penAppForm1CalcAvgSettingData() 
	{
	...
	@SuppressWarnings("unchecked")
	public int penAppForm1CalcdaSettingData(String dadate) {
	...
	@SuppressWarnings("unchecked")
	public Double penAppForm1CalcComSettingData(int age) {
	...
	public List<Object[]> penAppForm1CalcEmpStatusData(int empId) 
	{
	...
	public boolean savePensionAppForm1Calculation(PensionApplicationForm1Dao pensionAppform1)
	{		
		...
		{							
			trans = session.beginTransaction();					
			session.saveOrUpdate(pensionAppform1);
			trans.commit();
		} 	
	...
	public boolean saveAddAverageEmoluments(List<PensionApplicationForm1AverageEmolumentsDao> penform1averageemoluments) {
        
		Session session = SessionFactoryUtils.doGetSession(sessionFactory, true);
		Transaction trans = null;
		PensionApplicationForm1AverageEmolumentsDao addavgemoobj=null;
		try {	
			Iterator iter = penform1averageemoluments.iterator();
			trans = session.beginTransaction();
			Query maxcount = session.createQuery("select max(avgID) from PensionApplicationForm1AverageEmolumentsDao");
			while(iter!=null && iter.hasNext()){
				
				addavgemoobj = new PensionApplicationForm1AverageEmolumentsDao();
				...
				addavgemoobj =(PensionApplicationForm1AverageEmolumentsDao)iter.next();
...
public boolean saveAddOtherDepartmentService(List<PensionApplicationForm1OthDeptServiceDao> penform1otherdepartmentservice) {
		...
		Session session = SessionFactoryUtils.doGetSession(sessionFactory, true);
		Transaction trans = null;
		PensionApplicationForm1OthDeptServiceDao addavgothdeptseroobj=null;
		try {	
			//Iterator iter = penform1otherdepartmentservice.iterator();
			trans = session.beginTransaction();
			
			for(PensionApplicationForm1OthDeptServiceDao dao: penform1otherdepartmentservice)
			{
				session.saveOrUpdate(dao);
			}
			
			/*Query maxcount = session.createQuery("select max(SerID) from PensionApplicationForm1OthDeptServiceDao");
			while(iter!=null && iter.hasNext()){
				
				addavgothdeptseroobj = new PensionApplicationForm1OthDeptServiceDao();
				String maxavgemolid=null;
					try
					{
						maxavgemolid=maxcount.iterate().next().toString();
				...
				int maxavgemolid1 = maxavgemolid!=null?(Integer.parseInt(maxavgemolid))+1:1;
				addavgothdeptseroobj =(PensionApplicationForm1OthDeptServiceDao)iter.next();
				addavgothdeptseroobj.setSerID(maxavgemolid1);
				session.save(addavgothdeptseroobj);
	...
	public boolean saveForm1Recoveries(List<PensionApplicationForm1RecoveriesDao> form1pensionrecoveries) {       
		...
		Session session = SessionFactoryUtils.doGetSession(sessionFactory, true);
		Transaction trans = null;
		PensionApplicationForm1RecoveriesDao addrecobj=null;
		try {	
			Iterator iter = form1pensionrecoveries.iterator();
			trans = session.beginTransaction();
			Query maxcount = session.createQuery("select max(recID) from PensionApplicationForm1RecoveriesDao");
			while(iter!=null && iter.hasNext()){
				
				addrecobj = new PensionApplicationForm1RecoveriesDao();
				...
				addrecobj =(PensionApplicationForm1RecoveriesDao)iter.next();				
