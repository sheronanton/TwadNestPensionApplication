...
public class PensionApplicationHOForm1CalcMstValDataDaoImpl implements PensionApplicationHOForm1CalcMstValDataService{
	...
	public List<Object[]> penAppHOForm1MstValData(int loginEmpId, int curloginEmpId) 
	{
		...
		try		
		{
			...
			"left outer join HRM_PEN_APP_HO_CO_FORM1_DET f1 on f1.EMP_ID=emp.EMPLOYEE_ID " +
	...
	public List<Object[]> penAppHOForm1CalcOldMstValData(int loginEmpId) 
	{
	...
	public List<PensionApplicationHOForm1AverageEmolumentsDao> loadHOForm1OldAeVal(int empid) {
	...
	public List<PensionApplicationHOForm1RecoveriesDao> loadHOForm1OldRecoveriesVal(int empid) {
		// TODO Auto-generated method stub
	...
	public List<Object[]> penAppHOForm1CalcSettingValData() 
	{
	...
	public List<Object[]> penAppHOForm1CalcAvgSettingValData() 
	{
	...
	public List<Object[]> penAppHOForm1CalcEmpStatusValData(int empId) 
	{
	...
	public boolean savePensionAppHOForm1ValCalculation(PensionApplicationHOForm1ValDao pensionAppValHOform1)
	{		
		...
		{							
			trans = session.beginTransaction();					
			session.saveOrUpdate(pensionAppValHOform1);
			trans.commit();
		} 	
	...
	public boolean saveAddHOAverageEmolumentsVal(List<PensionApplicationHOForm1AverageEmolumentsValDao> valpenHOform1averageemoluments) {
        
		Session session = SessionFactoryUtils.doGetSession(sessionFactory, true);
		Transaction trans = null;
		PensionApplicationHOForm1AverageEmolumentsValDao addavgemoobj=null;
		try {	
			Iterator iter = valpenHOform1averageemoluments.iterator();
			trans = session.beginTransaction();
			Query maxcount = session.createQuery("select max(avgID) from PensionApplicationHOForm1AverageEmolumentsValDao");
			while(iter!=null && iter.hasNext()){
				
				addavgemoobj = new PensionApplicationHOForm1AverageEmolumentsValDao();
				...
				addavgemoobj =(PensionApplicationHOForm1AverageEmolumentsValDao)iter.next();
	...
	public boolean saveAddHOOtherDepartmentServiceVal(List<PensionApplicationHOForm1OthDeptServiceValDao> valHOform1OthdeptService)
	{
		...
		try {	
			...
			for(PensionApplicationHOForm1OthDeptServiceValDao dao: valHOform1OthdeptService)
			{
	...
	public boolean saveHOForm1RecoveriesVal(List<PensionApplicationHOForm1RecoveriesValDao> valHOform1recmultidatlist) {       
		
		Session session = SessionFactoryUtils.doGetSession(sessionFactory, true);
		Transaction trans = null;
		PensionApplicationHOForm1RecoveriesValDao addrecobj=null;
		try {	
			Iterator iter = valHOform1recmultidatlist.iterator();
			trans = session.beginTransaction();
			Query maxcount = session.createQuery("select max(recID) from PensionApplicationHOForm1RecoveriesValDao");
			while(iter!=null && iter.hasNext()){
				
				addrecobj = new PensionApplicationHOForm1RecoveriesValDao();
				...
				addrecobj =(PensionApplicationHOForm1RecoveriesValDao)iter.next();				
	...
	try{
		trans=session.beginTransaction();
		Query query = session.createSQLQuery("delete from HRM_PEN_APP_HO_MST_AVG_FORM1 where EMP_ID="+emp_id);		
		query.executeUpdate();
		trans.commit();
	...
	try{
		trans=session.beginTransaction();		
		Query query = session.createSQLQuery("delete from HRM_PEN_APP_HO_MST_REC_FORM1 where EMP_ID="+emp_id);		
		query.executeUpdate();
		trans.commit();
	...
	public boolean deleteHOForm1OthdeptdetailsVal(int emp_id)
	{
		...
		try{
			trans=session.beginTransaction();		
			Query query = session.createSQLQuery("delete from HRM_PEN_APP_HO_MST_OTHDEPTSER where EMPLOYEEID="+emp_id);		
			query.executeUpdate();
			trans.commit();
	...
	try{
		trans=session.beginTransaction();		
		Query query = session.createSQLQuery("delete from HRM_PEN_APP_HO_CO_FORM1_DET where EMP_ID="+emp_id);
		query.executeUpdate();
		trans.commit();
