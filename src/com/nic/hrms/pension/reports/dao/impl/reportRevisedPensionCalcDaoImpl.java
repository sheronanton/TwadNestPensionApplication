...
public class reportRevisedPensionCalcDaoImpl implements reportRevisedPensionCalcService{
		...
		for(Object[] avgsettingtmp:avgsettingmainli)
		{
			...
			if((avgsettingtmp[0]+"").equals("OTHER PAY3"))
			{
				...
				String sqlQry="select " +
						...
						"cd.FAM_PEN_AFTER_SEVEN, " +
						"cd.DEATH_DATE " +
						"from HRM_PEN_APP_REVISED_PEN_DET_CO cd where cd.EMP_ID="+empId;				
				List<Object[]> mainli=session.createSQLQuery(sqlQry).list();
				for(Object[] tmp:mainli)
				{
	...
	public List<pensionForm1CalcDetailsDao> getHOForm1OtherDeptService(HttpServletRequest request, int empId)
	{
		...
		System.out.println("inside other deept.........");
		Connection con=null;
	    Session session = SessionFactoryUtils.doGetSession(sessionFactory,true);
		...
		try
		   {
				...
				for(Object[] tmp:recli)
				{
					...
					System.out.println("inside other deept........."+tmp[0]+"");
