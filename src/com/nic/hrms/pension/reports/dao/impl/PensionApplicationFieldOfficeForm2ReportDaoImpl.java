...
public class PensionApplicationFieldOfficeForm2ReportDaoImpl implements PensionApplicationFieldOfficeForm2ReportService {

	private SessionFactory sessionFactory;

	
	@SuppressWarnings({ "unchecked", "deprecation" })
	public List<PensionApplicationFieldOfficeForm2ReportDao> printFieldOfficeDetails(HttpServletRequest request, int empId, int officeId) {
		...
		try
		{
			...
			String myQry="select det.EMP_NO,det.EMP_NAME,det.GENDER,det.DESIG_SERVICE_GRP,det.DESIG_ID,GRADE,det.OFFICE_ID,det.GPF_NO,det.FATHER_NAME," +
				...
				" det.NOT_VERIFY_SERVICE_TOT_DAYS,off.OFFICE_NAME,design.DESIGNATION,sta.STATE_NAME from HRM_PEN_APP_MST_FORM2 det left outer join COM_MST_OFFICES off " +
				" on off.OFFICE_ID=det.OFFICE_ID left outer join HRM_EMP_CURRENT_POSTING post ON post.EMPLOYEE_ID=det.emp_no left outer join HRM_MST_DESIGNATIONS design "+
				" on design.DESIGNATION_ID=post.DESIGNATION_ID " +
	...
	public List<PensionApplicationFieldOfficeNomineeForm2> printFieldOfficeNominee(HttpServletRequest request, int empIdList) 
	{
		Session session=SessionFactoryUtils.doGetSession(sessionFactory, true);
		...
		try
		{
			...
			String qry="SELECT emp_no,  nominee_id,  nominee_initial,  nominee_name,  relationship_id,  dob,"
						+ "age,  handicapped,  marital_status,  nomination_date,  active_status,  reason FROM HRM_PEN_APP_MST_FORM2_NOMIN where emp_no="+empIdList;
	...
	@SuppressWarnings({ "unchecked" })
	public List<PensionApplicationFieldOfficeNotVerServForm2> printFieldOfficeNVServ(HttpServletRequest request, int empIdList) 
	{
		System.out.println("Hi...inside servicedaoimpl..................");
		Session session=SessionFactoryUtils.doGetSession(sessionFactory, true);
		...
		try
		{
			...
			String qry2="SELECT  nvco.id,nvco.emp_no, nvco.start_date,  nvco.end_date,  nvco.reason,  nvco.not_verify_years,  nvco.not_verify_months,  nvco.not_verify_days, "
				...
				+ " FROM HRM_PEN_APP_MST_FORM2_NOTVER nvco left outer join HRM_PEN_APP_MST_FORM2 det on det.EMP_NO=nvco.emp_no"
