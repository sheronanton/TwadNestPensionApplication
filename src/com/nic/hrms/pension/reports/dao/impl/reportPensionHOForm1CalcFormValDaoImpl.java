...
public class reportPensionHOForm1CalcFormValDaoImpl implements reportPensionHOForm1CalcFormValService{

	private SessionFactory sessionFactory;
	private String forward="/t.c.f.b.o/",forward_officer="Administrative Officer,",forward_officer1="Head office ,",forward_officer2="Twad Board,Chennai-5";
	@SuppressWarnings({ "unchecked", "deprecation" })
	public List<pensionForm1CalcValDetailsDao> getHOForm1AvgValDetails(HttpServletRequest request, int empId) {
		...
		for(Object[] avgsettingtmp:avgsettingmainli)
		{
		 ...
		 String str="select sum(av.PROP_PAY) from HRM_PEN_APP_HO_MST_AVG_FORM1 av where av.EMP_ID="+empId;
		...
		String objstring="";
		 ...
		 averageemolamount=sumofav/aetotmonth;
				...
				String sqlQry="SELECT av.AVG_ID, av.NOT_CONSIDERED,  av.DATE_FROM,   av.DATE_TO,   av.TOT_MONTHS,  av.TOT_DAYS,  av.BASIC_PAY, av.GRADE_PAY, av.SPECIAL_PAY, av.OTHER1_PAY,  av.OTHER2_PAY, av.OTHER3_PAY, av.PROP_PAY, det.DATE_OF_RETIRE FROM HRM_PEN_APP_HO_MST_AVG_FORM1 av left outer join HRM_PEN_APP_HO_MST_FORM1_DET det on det.emp_id=av.EMP_ID WHERE av.EMP_ID="+empId+" ORDER BY av.DATE_FROM" ;
				...
				for(Object[] tmp:li)
				{
					...
					if(!(tmp[13]+"").equals("null"))
					{
						dis="Service verification upto "+formatter.format(tmp[13])+" to be furnished";
					}
					pd.setSUBJECT_CONTENT1(dis);
					BigDecimal avgpaydrwnobj=new BigDecimal(averageemolamount);
					pd.setAVGPAYDRWAN(avgpaydrwnobj);
					pd.setFORWARD(forward);
					pd.setFORWARD_OFFICER(forward_officer);
					pd.setFORWARD_OFFICER1(forward_officer1);
					pd.setFORWARD_OFFICER2(forward_officer2);
					retn.add(pd);			
		         }
...
//		 con.commit();        
	...
	public List<pensionForm1CalcValDetailsDao> getPensionHOForm1CalcValDetails(HttpServletRequest request, int empId) {
		...
		try
		   {
				...
				String subjectcontent5="";
				String subjectcontent6="";
				String subjectcontent7="";
		...
		for(Object[] avgsettingtmp:avgsettingmainli)
		{
			...
			if((avgsettingtmp[0]+"").equals("OTHER PAY3"))
			{
				...
				String sqlQry="select " +
						...
						"cd.LEVEL_OF_PAY   " +
						"from HRM_PEN_APP_HO_MST_FORM1_DET cd left outer join hrm_pen_app_authorization_org auth on cd.emp_id=auth.emp_no where cd.EMP_ID="+empId;				
				...
				for(Object[] tmp:mainli)
				{
					...
					mainpd.setLEVEL_OF_PAY((BigDecimal)tmp[91]);
					System.out.println("levelofpay::"+tmp[91]);
					...
					mainpd.setACTUAL_RET_DATE((Date)tmp[13]);
					if(vrsdateflag.equals("null") || vrsdateflag.equals(""))
					{
						...
						mainpd.setRETIREMENT_LABLE("Super Annuation Date");
						if(!(tmp[13]+"").equalsIgnoreCase("null"))
						{
						mainpd.setDATE_OF_RETIRE((Date)tmp[13]);
						subjectcontent="1. Service Verification upto "+formatter.format(tmp[13])+" to be furnished";
						subjectcontent5="Subject to approval necessary authorisation is put up below for approval with the following remarks";
					        mainpd.setSUBJECT_CONTENT5(subjectcontent5);
					        subjectcontent6="2. LPC to be furnished";
					        mainpd.setSUBJECT_CONTENT6(subjectcontent6);
					        subjectcontent7="3. Copy of Retirement order to be furnished";
					        mainpd.setSUBJECT_CONTENT7(subjectcontent7);
					        
					    mainpd.setSUBJECT_CONTENT1(subjectcontent);
						}
					...
					else
					{
						...
						mainpd.setRETIREMENT_LABLE("VRS Date");
						if(!(tmp[14]+"").equalsIgnoreCase("null"))
						{
						mainpd.setDATE_OF_RETIRE((Date)tmp[14]);
						subjectcontent="1. Service Verification upto "+formatter.format(tmp[14])+" to be furnished";
						 subjectcontent5="Subject to approval necessary authorisation is put up below for approval with the following remarks";
					        mainpd.setSUBJECT_CONTENT5(subjectcontent5);
					        subjectcontent6="2. LPC to be furnished";
					        mainpd.setSUBJECT_CONTENT6(subjectcontent6);
			       ...
			       // lastpaydrwan=lastbasic+lastgrade+lastspecial+lastother1+lastother2+lastother3;
					...
					try
					{
						...
						String objstring="";
						 ...
						 String sqlQryforsumofrec="select rec.REC_AMOUNT,rec.REC_DESC from HRM_PEN_APP_HO_MST_REC_FORM1 rec where rec.EMP_ID="+empId;		
							...
							if(sumofrec>0)
							{
								...
								mainpd.setDCRG_RECOVERIES("Recoveries = Rs. "+sumofrec+"/-");
								mainpd.setNET_DCRG("DCRG after deduction of Recovery = Rs. "+dcrgdisfinalamount+"/-");
							...
							if(sumofrec<=0)
							{
								mainpd.setDCRG_RECOVERIES("Recoveries = Nill");
							}
			        ...
			        //mainpd.setSUBJECT_CONTENT(subject_content);
			       
			        if((tmp[86]+"").equalsIgnoreCase("Final"))
			        {
...
//		 con.commit();
	...
	public List<pensionForm1CalcValDetailsDao> getHOForm1OtherDeptServiceVal(HttpServletRequest request, int empId)
	{
		Connection con=null;
	    Session session = SessionFactoryUtils.doGetSession(sessionFactory,true);
		List<pensionForm1CalcValDetailsDao> othdeptretn=new ArrayList<pensionForm1CalcValDetailsDao>();
		try
		   {
		        con=session.connection();	
				...
				{
					pensionForm1CalcValDetailsDao othdpetpd=new pensionForm1CalcValDetailsDao();
...
//		 con.commit();        	
	...
	@SuppressWarnings({ "deprecation", "unchecked" })
	public List<pensionForm1CalcValDetailsDao> getHOForm1RecoveryValDetails(HttpServletRequest request, int empId) {
		
		Connection con=null;
	    Session session = SessionFactoryUtils.doGetSession(sessionFactory,true);
		List<pensionForm1CalcValDetailsDao> recretn=new ArrayList<pensionForm1CalcValDetailsDao>();
		try
		   {
		        con=session.connection();	
				...
				String sqlQry="select rec.REC_ID,rec.REC_DESC,rec.REC_AMOUNT from HRM_PEN_APP_HO_MST_REC_FORM1 rec where rec.EMP_ID="+empId;
				
				List<Object[]> recli=session.createSQLQuery(sqlQry).list();
				for(Object[] tmp:recli)
				{
					pensionForm1CalcValDetailsDao recpd=new pensionForm1CalcValDetailsDao();
...
//		 con.commit();        	
	...
	@SuppressWarnings({ "deprecation", "unchecked" })
	public List<pensionForm1CalcValDetailsDeathDao> getPensionHOForm1CalcDeathValDetails(HttpServletRequest request, int empId) {
		
		Connection con=null;
	    Session session = SessionFactoryUtils.doGetSession(sessionFactory,true);
		List<pensionForm1CalcValDetailsDeathDao> deathmainretn=new ArrayList<pensionForm1CalcValDetailsDeathDao>();
		try
		   {
		        con=session.connection();	
		...
		for(Object[] avgsettingtmp:avgsettingmainli)
		{
			...
			if((avgsettingtmp[0]+"").equals("OTHER PAY3"))
			{
				...
				for(Object[] tmp:mainli)
				{
					pensionForm1CalcValDetailsDeathDao deathmainpd=new pensionForm1CalcValDetailsDeathDao();
					...
					if(tmp[87]!=null)
						{
							if((tmp[87]+"").equals("Yes"))
							{
								...
								otherdeptservice="Other Dept Service";
								deathmainpd.setOTHDEPTYEAR(othdeptyear+"");
								deathmainpd.setOTHDEPTMONTH(othdeptmonth+"");
								deathmainpd.setOTHDEPTDAY(othdeptday+"");
								deathmainpd.setOTHDEPTSERVICE(otherdeptservice);
							}
					   }
					if(wceflag.equals("Yes") && wcecountflag.equals("No"))
					{
					...
					deathmainpd.setACTUAL_RET_DATE((Date)tmp[13]);
					if(vrsdateflag.equals("null") || vrsdateflag.equals(""))
					{
						...
						deathmainpd.setRETIREMENT_LABLE("Death Date");
						...
						//subjectcontent="1.Service Verification upto "+formatter.format(tmp[86])+" to be furnished";
						//deathmainpd.setSUBJECT_CONTENT1(subjectcontent);
						}
					...
					else
					{
						...
						deathmainpd.setRETIREMENT_LABLE("VRS Date");
						if(!(tmp[14]+"").equalsIgnoreCase("null"))
						{
						...
						String objstring="";
						 ...
						 String sqlQryforsumofrec="select rec.REC_AMOUNT,rec.REC_DESC from HRM_PEN_APP_HO_MST_REC_FORM1 rec where rec.EMP_ID="+empId;		
							...
							if(sumofrec>0)
							{
								...
								deathmainpd.setDCRG_RECOVERIES("Recoveries = Rs. "+sumofrec+"/-");
								deathmainpd.setNET_DCRG("DCRG after deduction of Recovery = Rs. "+dcrgdisfinalamount+"/-");
															
							}
							if(sumofrec<=0)
							{
								deathmainpd.setDCRG_RECOVERIES("Recoveries = Nill");
							}
					...
					String subjectcontent5="Subject to approval necessary authorisation is put up below for approval ";
					deathmainpd.setSUBJECT_CONTENT5(subjectcontent5);
				//	deathmainpd.setSUBJECT_CONTENT(subject_content);
