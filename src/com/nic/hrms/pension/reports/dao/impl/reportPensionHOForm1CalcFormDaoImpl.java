...
public class reportPensionHOForm1CalcFormDaoImpl implements reportPensionHOForm1CalcFormService{

	private SessionFactory sessionFactory;

	@SuppressWarnings({ "unchecked", "deprecation" })
	public List<pensionForm1CalcDetailsDao> getHOForm1AvgDetails(HttpServletRequest request, int empId) {
		...
		for(Object[] avgsettingtmp:avgsettingmainli)
		{
		 ...
		 String str="select sum(av.PROP_PAY) from HRM_PEN_APP_HO_CO_A_EMOL_FORM1 av where av.EMP_ID="+empId;
		...
		String objstring="";
		 ...
		 averageemolamount=sumofav/aetotmonth;
				...
				String sqlQry="select av.AVG_ID,av.NOT_CONSIDERED,av.DATE_FROM,av.DATE_TO,av.TOT_MONTHS,av.TOT_DAYS,av.BASIC_PAY,av.GRADE_PAY,av.SPECIAL_PAY,av.OTHER1_PAY,av.OTHER2_PAY,av.OTHER3_PAY,av.PROP_PAY from HRM_PEN_APP_HO_CO_A_EMOL_FORM1 av where EMP_ID="+empId;
	...
	public List<pensionForm1CalcDetailsDao> getPensionHOForm1CalcDetails(HttpServletRequest request, int empId) {
		...
		for(Object[] avgsettingtmp:avgsettingmainli)
		{
			...
			if((avgsettingtmp[0]+"").equals("OTHER PAY3"))
			{
				...
				String sqlQry="select " +
						...
						"OTH_DEPT_TOT_DAYS ,  " +
						"LEVEL_OF_PAY   " +
						"from HRM_PEN_APP_HO_CO_FORM1_DET cd where cd.EMP_ID="+empId;				
				...
				for(Object[] tmp:mainli)
				{
					...
					System.out.println("level_of_pay::"+tmp[90]);
					...
					if(vrsdateflag.equals("null") || vrsdateflag.equals(""))
					{
						...
						subjectcontent="Service Verification upto "+formatter.format(tmp[13])+" to be furnished";
					...
					else
					{
						...
						subjectcontent="Service Verification upto "+formatter.format(tmp[14])+" to be furnished";
					    mainpd.setSUBJECT_CONTENT1(subjectcontent);
						}
	...
	public List<pensionForm1CalcDetailsDao> getHOForm1OtherDeptService(HttpServletRequest request, int empId)
	{
		Connection con=null;
	    Session session = SessionFactoryUtils.doGetSession(sessionFactory,true);
		...
		try
		   {
				...
				String sqlQry="SELECT " +
						...
						"TOT_DAY_SERVICE," +
						"DEPATMENT_NAME," +
						"REMARKS FROM HRM_PEN_APP_HO_CO_OTH_DEPT_SER where  EMPLOYEEID ="+empId;
	...
	@SuppressWarnings({ "deprecation", "unchecked" })
	public List<pensionForm1CalcDetailsDeathDao> getPensionHOForm1CalcDeathDetails(HttpServletRequest request, int empId) {
		...
		for(Object[] avgsettingtmp:avgsettingmainli)
		{
			...
			if((avgsettingtmp[0]+"").equals("OTHER PAY3"))
			{
				...
				for(Object[] tmp:mainli)
				{
					...
					deathmainpd.setLEVEL_OF_PAY((BigDecimal)tmp[91]);
					System.out.println("level::"+tmp[91]);
					String wceflag=tmp[21]+"";
					String wcecountflag=tmp[22]+"";
					...
					if(tmp[87]!=null)
				{
					...
					if(vrsdateflag.equals("null") || vrsdateflag.equals(""))
					{
						...
						subjectcontent="Service Verification upto "+formatter.format(tmp[86])+" to be furnished";
