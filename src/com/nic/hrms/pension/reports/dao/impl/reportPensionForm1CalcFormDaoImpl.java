...
public class reportPensionForm1CalcFormDaoImpl implements reportPensionForm1CalcFormService{

	private SessionFactory sessionFactory;

	@SuppressWarnings({ "unchecked", "deprecation" })
	public List<pensionForm1CalcDetailsDao> getForm1AvgDetails(HttpServletRequest request, int empId) {
		...
		for(Object[] avgsettingtmp:avgsettingmainli)
		{
		 ...
		 String str="select sum(av.PROP_PAY) from HRM_PEN_APP_AVG_EMOL_FORM1 av where av.EMP_ID="+empId;
		...
		String objstring="";
		 ...
		 averageemolamount=sumofav/aetotmonth;
				...
				String sqlQry="select av.AVG_ID,av.NOT_CONSIDERED,av.DATE_FROM,av.DATE_TO,av.TOT_MONTHS,av.TOT_DAYS,av.BASIC_PAY,av.GRADE_PAY,av.SPECIAL_PAY,av.OTHER1_PAY,av.OTHER2_PAY,av.OTHER3_PAY,av.PROP_PAY from HRM_PEN_APP_AVG_EMOL_FORM1 av where EMP_ID="+empId;
	...
	public List<pensionForm1CalcDetailsDao> getPensionForm1CalcDetails(HttpServletRequest request, int empId) {
		...
		try
		   {
		        ...
		        System.out.println("getPensionForm1CalcDetails::");
		...
		for(Object[] avgsettingtmp:avgsettingmainli)
		{
			...
			if((avgsettingtmp[0]+"").equals("LEVEL OF PAY"))
			{
				if((avgsettingtmp[1]+"").equals("Y"))
				{
					levelpayname=avgsettingtmp[3]+"";
				}
			}
				...
				String sqlQry="select " +
						...
						"OTH_DEPT_TOT_DAYS ,  " +
						"cd.LEVEL_OF_PAY   " +
						"from HRM_PEN_APP_FORM1_DETAILS cd where cd.EMP_ID="+empId;				
				...
				for(Object[] tmp:mainli)
				{
					...
					else
					{
						...
						vrs_fam_pen_text="Rs."+familypenfifty+"/- from the day following date of death of the pensioner for 10 years or upto "+uptodate+" whichever is earlier and Rs."+familypenthirty+"/- later on.";
					...
					try
					{
						...
						String objstring="";
						 ...
						 String sqlQryforsumofrec="select rec.REC_AMOUNT,rec.REC_DESC from HRM_PEN_APP_RECOVERY_FORM1 rec where rec.EMP_ID="+empId;		
...
//		 con.commit();
	...
	@SuppressWarnings({ "deprecation", "unchecked" })
	public List<pensionForm1CalcDetailsDao> getForm1OtherDeptService(HttpServletRequest request, int empId) {
		...
		try
		   {
				...
				String sqlQry="SELECT " +
						...
						"TOT_DAY_SERVICE," +
						"DEPATMENT_NAME," +
						"REMARKS FROM HRM_PEN_APP_MST_OTH_DEPT_SER where  EMPLOYEEID ="+empId;
	...
	@SuppressWarnings({ "deprecation", "unchecked" })
	public List<pensionForm1CalcDetailsDao> getForm1RecoveryDetails(HttpServletRequest request, int empId) {
		...
		try
		   {
				...
				String sqlQry="select rec.REC_ID,rec.REC_DESC,rec.REC_AMOUNT from HRM_PEN_APP_RECOVERY_FORM1 rec where rec.EMP_ID="+empId;
	...
	@SuppressWarnings({ "deprecation", "unchecked" })
	public List<pensionForm1CalcDetailsDeathDao> getPensionForm1CalcDeathDetails(HttpServletRequest request, int empId) {
...
//                   int k=(Integer) tmp[90];
                   
					int k = ((BigDecimal) tmp[90]).intValue();
                   System.out.println("k::"+k);
					...
					System.out.println("levelofpay::2------------->"+tmp[90]);
					...
					System.out.println("levelofpay::1"+tmp[90]);
					...
					System.out.println("levelofpay::"+tmp[90]);
					...
					try
					{
					if(!tmp[84].equals(null)||!tmp[84].equals("null"))
						uptodate=formatter1.format(tmp[84]);
