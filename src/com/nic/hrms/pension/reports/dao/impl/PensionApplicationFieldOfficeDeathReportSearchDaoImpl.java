...
public class PensionApplicationFieldOfficeDeathReportSearchDaoImpl implements PensionApplicationFeildOfficeDeathReportSearchService {
	
	private SessionFactory sessionFactory;

	
	@SuppressWarnings("unchecked")
	public List<Object[]> penAppFieldOfficeDeathReportUserSearch(String penappreportSearchOptions,String penappreportSearchKeyword,int empId ) {
		...
		if(penappreportSearchOptions.equals("1"))
		{
			...
			try
			{
				...
				/* String str="SELECT emp.employee_id," +
			 		...
			 		"LEFT OUTER JOIN HRM_PEN_APP_FAM_MST_DET_FORM2 fammstform2 on app.emp_id=fammstform2.emp_no " +
			 		...
			 		"(SELECT emp_id FROM HRM_PEN_APP_MST_FORM1_DET WHERE process_status = 'VALIDATE') AND app.death_date IS NOT NULL " +
			 		"AND fammstform2.EMP_NO IN " +
			 		"(SELECT EMP_NO FROM HRM_PEN_APP_FAM_MST_DET_FORM2 WHERE process_status = 'VALIDATE') " +
				 ...
				 String str="SELECT emp.employee_id," +
			 		...
			 		"SELECT employee_id FROM hrm_emp_controlling_office) AND employee_id LIKE '%"+penappreportSearchKeyword+"%')) "+
			 		"AND emp.employee_id IN " +
			 		"(SELECT emp_id FROM HRM_PEN_APP_MST_FORM1_DET WHERE process_status = 'VALIDATE') AND app.death_date IS NOT NULL " +
			 		"AND fammstform2.EMP_NO IN " +
			 		"(SELECT EMP_NO FROM HRM_PEN_APP_FAM_MST_DET_FORM2 WHERE process_status = 'VALIDATE') " +
			...
			try
			{
				...
				/* String str="SELECT emp.employee_id," +
				 ...
				 String str="SELECT emp.employee_id," +
			 		...
			 		"SELECT employee_id FROM hrm_mst_employees WHERE lower(employee_name) LIKE '%"+penappreportSearchKeyword.toLowerCase()+"%')))"+
			 		"AND emp.employee_id IN " +
			 		"(SELECT emp_id FROM HRM_PEN_APP_MST_FORM1_DET WHERE process_status = 'VALIDATE') AND app.death_date IS NOT NULL " +
			 		"AND fammstform2.EMP_NO IN " +
			 		"(SELECT EMP_NO FROM HRM_PEN_APP_FAM_MST_DET_FORM2 WHERE process_status = 'VALIDATE') " +
	...
	@SuppressWarnings("unchecked")
	public List<Object[]> penAppFieldOfficeDeathReportUserSearch1(String penappreportSearchKeyword,int empId ) {
		List<Object[]> list=null;		
