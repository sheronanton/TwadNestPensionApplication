...
function comListSearch(command)
{
	
	var opt1=document.getElementById("comListSearchOptions");
	var srctxt1=document.getElementById("comListSearchKeyword");
...
if(!srctxt1.value.match(alphaExp))
{
	...
	alert('Please Enter Alphabets Only');
		...
		{
		comListSearch1(command);		
		}
}
	
	function comListSearch1(command)
	{		
		var comListSearchOptions=document.forms[0].comListSearchOptions.value;
		var comListSearchKeyword=document.forms[0].comListSearchKeyword.value;
	    if(command=="Search")
		  {
   		   var url="../commutationListSearch.html?comList.comListSearchKeyword="+comListSearchKeyword+
   		   "&comList.comListSearchOptions="+comListSearchOptions;
	...
	//hguigiguigugigu
	function commstListSearch(command)
	{
	...
	if(!srctxt1.value.match(alphaExp))
	{
		...
		alert('Please Enter Alphabets Only');
			...
			{
			comListSearch2(command);		
			}
	}
		
		function comListSearch2(command)
		{		
			var comListSearchOptions=document.forms[0].comListSearchOptions.value;
			var comListSearchKeyword=document.forms[0].comListSearchKeyword.value;
		    if(command=="Search")
			  {
	   		   var url="../commutationmstListSearch.html?comList.comListSearchKeyword="+comListSearchKeyword+
	   		   "&comList.comListSearchOptions="+comListSearchOptions;
	...
	function processResponsecomList(req)
	{   
      if(req.readyState==4)
	  {
	      if(req.status==200)
	...
	function getRowcomList(baseResponse)
	{   
		  ...
		  if(len==0)
          {
   	      ...
   	      document.getElementById("comListSearchKeyword").value="";
   	      cleanTbodycomCheck();
  	      }
		   if(flag=='true')
         	 {
