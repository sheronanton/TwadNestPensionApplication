...
function famPenDetailSearch(command)
{
	...
	else{famPenDetailSearch1(command);}
	}
...
if(!srctxt1.value.match(alphaExp))
{
	...
	alert('Please Enter Alphabets Only');
		...
		{
		famPenDetailSearch1(command);		
		}
}
	
	function famPenDetailSearch1(command)
	{		
	    ...
	    if(command=="Search")
		  {
	    	var url="photoWithSignDetailPhSearch.html?famvalpenreptph.reportSearchKeyword="+reportSearchKeyword+
   		   "&famvalpenreptph.reportSearchOptions="+reportSearchOptions;
   		 
		   var req=getTransport1();
           req.open("GET",url,true);        
           req.onreadystatechange=function()
           {
               processResponsepenDetail(req);
            }   
            req.send(null);
            }  
 	}
	...
	function processResponsepenDetail(req)
	{   
      if(req.readyState==4)
	  {
	      if(req.status==200)
	      {  
	       ...
	       if(command=="FamPenProfileSearch")
	        {        
	    	   getRowPenDetail(baseResponse); 
	...
	function getRowPenDetail(baseResponse)
	{   
		  ...
		  if(len==0)
          {
   	      ...
   	      document.getElementById("reportSearchKeyword").value="";
   	      cleanTbodyPenDetail();
  	      }
		   if(flag=='true')
         	 {
