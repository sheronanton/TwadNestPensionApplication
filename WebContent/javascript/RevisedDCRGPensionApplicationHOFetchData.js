...
function fetchMstDataCheckVal(empNo)
{
	...
	{
		var url="LoadRevDCRGPenAppHeadData.html?revisedDcrgPenappsearch.empId="+empId;	
...
{
		var empNo=document.getElementById("empNo").value;
		var url="revpenAppHOFetchOldMstValdata.html?revisedDcrgPenappsearch.empId="+empNo;	 
	    	  ...
	    	  else
	    	  {
		    	  ...
		    	  for(var j=0;j<olddisplay.length;j++)
		    	  {
		    		document.getElementById("scaleofpay").value=nullcheck(oldbaseResponse.getElementsByTagName("SCALE_OF_PAY")[j].firstChild.nodeValue);
	           		...
	           		var doprov=nullcheck(oldbaseResponse.getElementsByTagName("DATE_OF_PROVINC")[j].firstChild.nodeValue);
	           		
	           		
	           		if(doprov!="")
	           		{	           			
	           			document.getElementById("cal-button-9").style.display="none";
	           		}
	           		else
	           		{
	           			document.getElementById("cal-button-9").style.display="none";
...
function loadOldAE()
{
	...
	var empNo=document.getElementById("empNo").value;	
	var url='revpenAppHOFetchOldAEValdata.html?revisedDcrgPenappsearch.empId='+empNo;	
				...
				else
				{
					...
					for(var i=0;i<aeservice.length;i++)
					{
						...
						if((aeservice.length)==1)
						{
							...
							document.getElementById("maxcheck").disabled=true;
...
function ChangeAeGrid(nc,fromdate,todate,totmonth,totdays,basic,grade,special,other1,other2,other3,totalamount,ppamount)
{
  ...
  if (window.navigator.appName.toLowerCase().indexOf("netscape") == -1)
	{     	
		if(nc=='Y')
		{
			hiddencheck=document.createElement("<input type='checkbox' name='maxcheck' id='maxcheck' checked='checked' value='"+rid+"' onclick='checkfun1(this.value)' disabled='true' />");
		}
		else
		{
		hiddencheck=document.createElement("<input type='checkbox' name='maxcheck' id='maxcheck' value='"+rid+"' onclick='checkfun1(this.value)' disabled='true' />");
		}
