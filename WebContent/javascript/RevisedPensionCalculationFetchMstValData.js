...
{	
		var url="revisedPenCalcFetchSettingValData.html";		
...
function fetchMstDataCheckVal(empNo)
{
	...
	{
		var url="revisedPenCalculationFetchChangeData.html?revPensionValCalculation.empNo="+empId;
	 ...
	 if(chvalreq.readyState==4)
	  {
	      ...
	      { 
	    	  ...
	    	  {
	    		  alert('Not Valid Employee Number for Revise.');
...
function fetchMstData(empNo)
{
	...
	if(empId!="" && empId.length>0)
	{
		var url="revisedPenCalculationChangeData.html?revPensionValCalculation.empNo="+empId;	 
		var req=getTransportWth();	    
		req.open("GET",url,true);        
	    req.onreadystatechange=function()
	       {
...
function fetchCalcOldData()
{
		...
		var url="revisedPenCalculationChangeDataLoad.html?revPensionValCalculation.empNo="+empNo;	
	    //alert(url);
		    	  ...
		    	  for(var j=0;j<olddisplay.length;j++)
		    	  {
		    		document.getElementById("scaleofpay").value=nullcheck(oldbaseResponse.getElementsByTagName("SCALE_OF_PAY")[j].firstChild.nodeValue);
	           		...
	           		if(dovrs!="")
	           		{
	           			...
	           			vrs_date_calc();
	           			//alert("After vrs calac");
	           		}
	           		else
	           		{
	           			document.getElementById("vrs_date").value="";
	           		}	           		
	           		//alert("vrs date"+dovrs);
...
function loadHOOldOtherDeptService()
{
	//alert("other dept");
	var empNo = document.getElementById("empNo").value;
	var url = 'RevisedPensionCalcOldOthDeptServValdata.html?revPensionValCalculation.empNo=' + empNo;
...
function loadOldAE()
{
	...
	var empNo=document.getElementById("empNo").value;	
	var url='revisedPenCalculationFetchChangeAEdata.html?revPensionValCalculation.empNo='+empNo;	
	var HttpAERequest = getTransportWth();
	HttpAERequest.open("GET", url, true);
	HttpAERequest.onreadystatechange=function()
     {    
...
{
		var empNo=document.getElementById("empNo").value;
		var url="penAppHOForm1CalcFetchFieldOfficeOldMstdata.html?revPensionValCalculation.empNo="+empNo;	 
...
function loadFieldOFficeOldAE()
{
	...
	var empNo=document.getElementById("empNo").value;	
	var url='penAppHOForm1CalcFetchFieldOfficeOldAEdata.html?revPensionValCalculation.empNo='+empNo;	
...
function fetchCommutedvalueData()
{
	...
	//alert(age);
		var url="comRevpenCalcFetchMstValdata.html?revPensionValCalculation.age="+age;	 
...
function fetchEmployeeStatusData()
{
	...
	{
		var url="empStatusRevPensionCalcFetchMstValdata.html?revPensionValCalculation.classPensionId="+typepension+"&revPensionValCalculation.empNo="+empNo;	 
...
function validation_final()
{
	...
	if((penvalflag==1) || (totservalflag==1) || (lastvalflag==1) || (aevalflag==1) || (eligiblevalflag==1))
	{
		return false;
	}
	else
	{
		 ...
		 try
		  ...
		  //alert("pensionamount::"+pensionamount+"pencal::"+pencalamount+"fampensionamountbeforceiling::"+fampensionamountbeforceiling+"pensionyearcal::"+pensionyearcal);
