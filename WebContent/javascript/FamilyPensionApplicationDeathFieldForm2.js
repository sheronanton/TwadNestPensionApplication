...
function checkAvailForm2(empId)
{	
	if(empId!="" && empId.length>0)
		{
		var url="famPenFieldForm2CheckForm2.html?fchkavail.empId="+empId+"&rnd="+new Date().getTime();				
		var req2=getReq();	    
	...
	if(req2.readyState==4)
	  {
	      ...
	      {  
	    	  ...
	    	  if(display.length>0)
	    		  ...
	    		  {
	    		  	var empId=document.getElementById("penEmpNo").value;	    	
	    		  	checkAvailForm1(empId);	    		  
	    		  }
...
function checkAvailForm1(empId)
{	
	if(empId!="" && empId.length>0)
		{
		var url="famPenFieldForm2CheckForm1.html?fchkavail.empId="+empId+"&rnd="+new Date().getTime();				
		var req2=getReq();	    
		req2.open("GET",url,true);        
	    req2.onreadystatechange=function()
	       {
...
function fetchMstValue(val)
{
	if(val!="")
		{
			var url="famPenAppFieldForm2GetExistingData.html?getMstSearch.empId="+val+"&rnd="+new Date().getTime();			
			var ajaxObj=getReq();			
...
function loadMstData(baseResponse)
{
	  ...
	  {           	  
	 	...
	 	var relId=nullcheck(baseResponse.getElementsByTagName("religionID")[i].firstChild.nodeValue);
	 	if(relId=="")
	 		{
	 		...
	 		{
	 			document.getElementById("religionId").value=relId;
	 		}
	 	...
	 	var staId=nullcheck(baseResponse.getElementsByTagName("stateId")[i].firstChild.nodeValue);
	 	if(staId=="")
	 		{
	 		...
	 		{
	 			document.getElementById("stateId").value=staId;
	 		}
	 	
	 	
	 	var payId=nullcheck(baseResponse.getElementsByTagName("paymentOfficeId")[i].firstChild.nodeValue);
	 	if(payId=="")
	 		{
	 		...
	 		{
	 			document.getElementById("paymentOfficeId").value=payId;
	 		}
...
function setNomineeDetails(nomId,nomInit,nomName,nomRel,nomDob,nomAge,handy,marStatus,nomDate,nomStatus,nomAdd)
{
    ...
    var url="javascript:DeleteGrid('" +seq+"r')";              
    anc.href = url;
