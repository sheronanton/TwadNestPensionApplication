   ...
   function searchIt(command)
	{
	    ...
	    if(command=="Search")
		  {
          var url="designationSearch.html?desigsearch.searchKeyword="+SearchKeyword+"&desigsearch.searchOption="+SearchOptions;          
		  var req=getTransport1();
           req.open("GET",url,true);        
           req.onreadystatechange=function()
           {
               processResponseItSear(req);
            };   
            req.send(null);
            }  
 	}
		
	function processResponseItSear(req)
	{   
      if(req.readyState==4)
	  {
	      if(req.status==200)
	...
	{	
	    ...
	    totalblock=0;
	        ...
	        if(record2.length>0)
	        {
	            totalblock=parseInt(record2.length/__pagination);
	            if(record2.length%__pagination!=0)
	            {
               ...
               if(len>0)
                {
	              ...
	              for(var i=0;i<display.length;i++)
	                {
	                //record1[i] = nullcheck(baseResponse.getElementsByTagName("ppoNo")[i].firstChild.nodeValue);
	                record2[i] = baseResponse.getElementsByTagName("employeeId")[i].firstChild.nodeValue;
	                record3[i] = nullcheck(baseResponse.getElementsByTagName("pensionerName")[i].firstChild.nodeValue);
	                record4[i] = baseResponse.getElementsByTagName("designation")[i].firstChild.nodeValue;
	...
	function loadRecordVal(page)
	{
	    ...
	    for(i=p;i<record2.length && c<__pagination;i++)
	    {
	            c++;
	            sno++;
	            ...
	            cell3.setAttribute("id", "designation"+sno);
