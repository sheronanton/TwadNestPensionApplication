...
{	
	if(val!="" || val.length!=0)
	{
		...
		var url="familyPenAuthoriseGetMasterData.html?searchModel.empId="+val;
		var ajaxObj=getReq();
		ajaxObj.open("GET", url, true);
		ajaxObj.onreadystatechange=function()
		{
...
{	
	...
	{		  
		...
		{    
			...
			if(baseResponse.getElementsByTagName("authOfficerAddress")[i].firstChild!=null){
				document.getElementById("authOfficeAddress").value=nullcheck(baseResponse.getElementsByTagName("authOfficerAddress")[i].firstChild.nodeValue);
			}else{
		...
		{	
			...
			{    
				...
				//alert("NOMINEE ID"+baseResponse.getElementsByTagName("nomId")[i].firstChild.nodeValue);
				//alert("NOMINEE NAME"+baseResponse.getElementsByTagName("nomName")[i].firstChild.nodeValue);
				document.getElementById("nomineeId").value=nullcheck(baseResponse.getElementsByTagName("nomId")[i].firstChild.nodeValue);
				document.getElementById("nomineeName").value=nullcheck(baseResponse.getElementsByTagName("nomName")[i].firstChild.nodeValue);
				var option1=document.createElement("option");
				option1.value=nomId;
				option1.innerHTML=nomName;			 
				nomlist.appendChild(option1);
				//sel=nullcheck(baseResponse.getElementsByTagName("nomId")[i].firstChild.nodeValue);
			}
...
function getRelationName(myObj)
{
	...
	if(optValue!=0)
	{		
		var url="familyPenAuthorisegetNomRelation.html?searchModel5.empId="+empNo+"&searchModel5.keyword="+optValue;		
		var ajaxObj=getReq();
		ajaxObj.open("GET", url, true);
		ajaxObj.onreadystatechange=function()
		{
