...
{	
		var url="penAppHOForm1FetchSettingValdata.html";		
...
function fetchMstData(empNo)
{
	...
	if(empId!="" && empId.length>0)
	{
		var url="penAppHOForm1FetchMstValdata.html?pensionAppCalcValHOForm1.empNo="+empId;	 
		var req=getTransportWth();	    
		req.open("GET",url,true);        
	    req.onreadystatechange=function()
	       {
	 ...
	 if(req.readyState==4)
	  {
	      ...
	      { 
	    	  ...
	    	  if((display.length <=0) || flag=="false" || flag==false || displayflag==1)
	    	  {
	    		  	alert('Not a valid Employee no for validate.');
	    		  	//defaultselection();
	    		  	window.location.reload();
	    	  }
	    	  else
...
function fetchCalcOldData()
{
		...
		var url="penAppHOForm1CalcFetchOldMstValdata.html?pensionAppCalcValHOForm1.empNo="+empNo;	 
...
function loadHOOldOtherDeptService()
{
	var empNo = document.getElementById("empNo").value;
	var url = 'penAppHOForm1CalcOldOthDeptServValdata.html?pensionAppCalcValHOForm1.empNo=' + empNo;
		...
		if (HttpAERequest.readyState == 4) {
			...
			if (deptrid == 0)
			{
				...
				//	var remark = nullcheck(othdeptservice[i]
					...
					if(othdeptservice[i]
							.getElementsByTagName("remark")[0].firstChild == null){
								var remark = "";
							}else{
							var remark = othdeptservice[i]
							.getElementsByTagName("remark")[0].firstChild.nodeValue	
							}
...
function loadOldAE()
{
	...
	var empNo=document.getElementById("empNo").value;	
	var url='penAppHOForm1CalcFetchOldAEValdata.html?pensionAppCalcValHOForm1.empNo='+empNo;	
	var HttpAERequest = getTransportWth();
	HttpAERequest.open("GET", url, true);
	HttpAERequest.onreadystatechange=function()
     {    
...
{
	var empNo=document.getElementById("empNo").value;	
	var url='penAppHOForm1CalcFetchOldRecValdata.html?pensionAppCalcValHOForm1.empNo='+empNo+"&rid="+Math.random();
...
/*function fetchCommutedvalueData()
{
	...
	//alert(age);
		var url="compenAppHOForm1CalcFetchMstValdata.html?pensionAppCalcValHOForm1.age="+age;	 
...
function fetchEmployeeStatusData()
{
	...
	{
		var url="empStatusAppHOForm1CalcFetchMstValdata.html?pensionAppCalcValHOForm1.classPensionId="+typepension+"&pensionAppCalcValHOForm1.empNo="+empNo;	 
	...
	{  
		 try
		  ...
		  if((document.getElementById("pensionpertFlagpert").checked==true) && (document.getElementById("commPert").value!="") && (document.getElementById("pensionCommutedFlagYes").checked==true))
		  {
			  ...
			  commutedamount=Math.floor(parseInt((parseInt(pensionamount)*parseFloat(compert/100))));
...
function ae_form_final_validation()
{
	try
	{
		...
		{			
			...
			if((finalbasicpay!=lastbasicpay)|| (finalspecialpay!=lastspecialpay) || (finaloptionpay1!=lastother1pay) || (finaloptionpay2!=lastother2pay) || (finaloptionpay3!=lastother3pay))
			{
