...
{	
		var url="penAppHOForm1FetchSettingdata.html";		
...
function fetchMstDataCheckVal(empNo)
{
	...
	{
		var url="penAppHOForm1FetchMstCheckValdata.html?pensionAppCalcHOForm1.empNo="+empId;	 
	 ...
	 if(chvalreq.readyState==4)
	  {
	      ...
	      { 
	    	  ...
	    	  {
	    	  var displayflag=0;  
	    	  var baseResponse=chvalreq.responseXML.getElementsByTagName("response")[0];	    	  	    	  
	    	  var display=baseResponse.getElementsByTagName("record"); 
...
function fetchMstData(empNo)
{
	...
	if(empId!="" && empId.length>0)
	{
		var url="penAppHOForm1FetchMstdata.html?pensionAppCalcHOForm1.empNo="+empId;	 
		var req=getTransportWth();	    
		req.open("GET",url,true);        
	    req.onreadystatechange=function()
	       {
	 ...
	 if(req.readyState==4)
	  {
	      ...
	      { 
	    	  ...
	    	  {          		
	    		  ...
	    		  document.getElementById("desigId").value=nullcheck(baseResponse.getElementsByTagName("desigid")[i].firstChild.nodeValue);
	    		  document.getElementById("desigServiceGrp").value=nullcheck(baseResponse.getElementsByTagName("desigservgrp")[i].firstChild.nodeValue);
	    		  var empolyeestatus=nullcheck(baseResponse.getElementsByTagName("empStatusId")[i].firstChild.nodeValue);
	    		  if(empolyeestatus!="")
    			  {
	    			//  alert("from Current posting");
		    		  ...
		    		  /* 
		    		   if((empolyeestatus.toLowerCase()=="san") || (empolyeestatus.toLowerCase()=="wkg"))
		    		  {
		    			  document.getElementById("classPensionId").disabled=false;
		    		  } 
		    		  */
...
{
	//alert("towards CO table");
		var empNo=document.getElementById("empNo").value;
		var url="penAppHOForm1CalcFetchOldMstdata.html?pensionAppCalcHOForm1.empNo="+empNo;	 
	//	var url="penAppHOForm1CalcFetchFieldOfficeOldMstdata.html?pensionAppCalcHOForm1.empNo="+empNo; 
	    		...
	    		// alert(" from HO_CO ");
		    	  for(var j=0;j<olddisplay.length;j++)
		    	  {
		    		document.getElementById("scaleofpay").value=nullcheck(oldbaseResponse.getElementsByTagName("SCALE_OF_PAY")[j].firstChild.nodeValue);
	           		...
	           		//alert(nullcheck(oldbaseResponse.getElementsByTagName("PENSION_TYPE")[j].firstChild.nodeValue));
	           		...
	           		//alert("Count in other penAppProcessCalcOldResponse:"+count++);
	           		//calctotalnqs();
	           		calcservice();
	           		//calcworkservice();
	           		...
	           		var lasttotal=parseInt(lastbasic)+parseInt(lastspecial)+parseInt(lastother1)+parseInt(lastother2)+parseInt(lastother3);
					...
					if(serflag=="Yes")
						...
						try
						{
							//v loadHOOfficeOldOtherDeptService();
							loadFieldOfficeOldOtherDeptService();
...
function loadOldAE()
{
	//alert("from HO_CO emol");
	var empNo=document.getElementById("empNo").value;	
	var url='penAppHOForm1CalcFetchOldAEdata.html?pensionAppCalcHOForm1.empNo='+empNo;	
			...
			//Clearing the Existing data
				...
				else
				{
					...
					for(var i=0;i<aeservice.length;i++)
					{
						...
						totalamount=parseInt(basicpay)+parseInt(specialpay)+parseInt(optionpay1)+parseInt(optionpay2)+parseInt(optionpay3);
...
//field office old record start



function fetchCalcFieldOfficeOldData()
{
	//alert("towards MST table");
		var empNo=document.getElementById("empNo").value;
		var url="penAppHOForm1CalcFetchFieldOfficeOldMstdata.html?pensionAppCalcHOForm1.empNo="+empNo;	 
		var oldfieldofficereq=getTransportWth();
		document.getElementById("empNo").value=empNo;
		oldfieldofficereq.open("GET",url,true);        
		oldfieldofficereq.onreadystatechange=function()
	       {
			penAppProcessCalcFieldOfficeOldResponse(oldfieldofficereq);
	       };   
	       oldfieldofficereq.send(null);	     		
}
...
function penAppProcessCalcFieldOfficeOldResponse(oldfieldofficereq)
{
	
	if(oldfieldofficereq.readyState==4)
	  {		
		  if(oldfieldofficereq.status==200)
	      { 
			  var oldflagcheck=0;
			 try{
			  var oldbaseResponse=oldfieldofficereq.responseXML.getElementsByTagName("oldresponse")[0];
	    		...
	    		// alert("from MST ");
		    	  for(var j=0;j<olddisplay.length;j++)
		    	  {
		    		document.getElementById("scaleofpay").value=nullcheck(oldbaseResponse.getElementsByTagName("SCALE_OF_PAY")[j].firstChild.nodeValue);
	           		...
	           		document.getElementById("fs_month").value=nullcheck(oldbaseResponse.getElementsByTagName("FOREIGN_SERV_MONTH")[j].firstChild.nodeValue);
	           		document.getElementById("fs_year").value=nullcheck(oldbaseResponse.getElementsByTagName("FOREIGN_SERV_YEAR")[j].firstChild.nodeValue);
	           		//alert("Count in other penAppProcessCalcFieldOfficeOldResponse:"+count++);
	           		//calctotalnqs();
	           		calcservice();
...
function loadHOOfficeOldOtherDeptService()
{
	//alert("from HO_CO otherdept");
	var empNo = document.getElementById("empNo").value;
	var url = 'penAppHOForm1CalcFetchOldOthDeptServdata.html?pensionAppCalcHOForm1.empNo='+empNo;
	var HttpAERequest = getTransportWth();
	HttpAERequest.open("GET", url, true);
	HttpAERequest.onreadystatechange = function() {
		if (HttpAERequest.readyState == 4) {
			...
			if (deptrid == 0)
			{
				...
				//alert(rid);
					...
					if (window.navigator.appName.toLowerCase().indexOf("netscape") == -1)
					{
						...
						hidden2=document.createElement("<input type='text'  name='othdepttodate' id='othdepttodate"+rid+"' size='10' maxlength='10' value='"+todate+"' onblur='checkotherdeptdate("+rid+");' onkeypress='dtval(this,event)' class='aetextbox' >");        	   	
					...
					if (window.navigator.appName.toLowerCase().indexOf("netscape") == -1)
					{
						...
						hidden3=document.createElement("<input type='text'  name='otdepyear' id='otdepyear"+rid+"' value='"+totyear+"' size='4' maxlength='4'   readonly='true' style='background-color:#ececec' class='aetextboxmonth' >");        	   	
...
function loadFieldOfficeOldOtherDeptService()
{
//	alert("from MST otherdept");
	var empNo = document.getElementById("empNo").value;
	var url = 'penAppFieldOfficeForm1CalcFetchOldOthDeptServdata.html?pensionAppCalcHOForm1.empNo='+empNo;
	var HttpAERequest = getTransportWth();
	HttpAERequest.open("GET", url, true);
	HttpAERequest.onreadystatechange = function() {
		if (HttpAERequest.readyState == 4) {
			...
			if (deptrid == 0)
			{
				...
				//alert(rid);
					...
					if (window.navigator.appName.toLowerCase().indexOf("netscape") == -1)
					{
							hidden1=document.createElement("<input type='text'  name='othdeptfromdate' onkeypress='dtval(this,event)' id='othdeptfromdate'"+rid+"' size='10' maxlength='10'  value=''  class='aetextbox' >");
					...
					if (window.navigator.appName.toLowerCase().indexOf("netscape") == -1)
					{
						...
						hidden2=document.createElement("<input type='text'  name='othdepttodate' id='othdepttodate'"+rid+"' size='10' maxlength='10' value='' onkeypress='dtval(this,event)' class='aetextbox' >");        	   	
					...
					if (window.navigator.appName.toLowerCase().indexOf("netscape") == -1)
					{
						...
						hidden3=document.createElement("<input type='text'  name='otdepyear' id='otdepyear'"+rid+"' size='4' maxlength='4'   readonly='true' style='background-color:#ececec' class='aetextboxmonth' >");        	   	
					...
					if (window.navigator.appName.toLowerCase().indexOf("netscape") == -1)
					{
						...
						hidden4=document.createElement("<input type='text'  name='otdepmonth' id='otdepmonth'"+rid+"' size='4' maxlength='2' onkeypress='dtval(this,event)'  readonly='true' style='background-color:#ececec' class='aetextboxmonth' >");        	   	
					...
					if (window.navigator.appName.toLowerCase().indexOf("netscape") == -1)
					{
						...
						hidden5=document.createElement("<input type='text'  name='othdeptday' id='othdeptday'"+rid+"' size='4' maxlength='2' readonly='true' style='background-color:#ececec' class='aetextboxmonth' >");        	   	
					...
					if (window.navigator.appName.toLowerCase().indexOf("netscape") == -1)
					{
						...
						hidden6=document.createElement("<input type='text'  name='Departmentname' id='Departmentname'"+rid+"' size='10'   class='recdescriptionbox' >");        	   	
					...
					if (window.navigator.appName.toLowerCase().indexOf("netscape") == -1)
					{
						...
						hidden7=document.createElement("<input type='text'  name='Remarks' id='Remarks'"+rid+"' size='10'  class='recdescriptionbox' >");        	   	
...
function loadFieldOFficeOldAE()
{
//	alert("from MST emol");
	var empNo=document.getElementById("empNo").value;	
	var url='penAppHOForm1CalcFetchFieldOfficeOldAEdata.html?pensionAppCalcHOForm1.empNo='+empNo;	
	var HttpFieldOfficeAERequest = getTransportWth();
	HttpFieldOfficeAERequest.open("GET", url, true);
	HttpFieldOfficeAERequest.onreadystatechange=function()
     {    
		if(HttpFieldOfficeAERequest.readyState == 4)
		{		
			
			var aeresponse=HttpFieldOfficeAERequest.responseXML.getElementsByTagName("aeresponse")[0];
     ...
     HttpFieldOfficeAERequest.send(null);
}
...
function loadFieldOfficeOldRecovery()
{
	//alert("from MST recovery");
	var empNo=document.getElementById("empNo").value;	
	var url='penAppHOForm1CalcFetchFieldOfficeOldRecdata.html?pensionAppCalcHOForm1.empNo='+empNo+"&rid="+Math.random();
	var HttpFiledOfficeRecRequest = getTransportWth();
	HttpFiledOfficeRecRequest.open("GET", url, true);
	HttpFiledOfficeRecRequest.onreadystatechange=function()
     {    
		if(HttpFiledOfficeRecRequest.readyState == 4)
		{			
			
			var recresponse=HttpFiledOfficeRecRequest.responseXML.getElementsByTagName("recresponse")[0];		
     ...
     HttpFiledOfficeRecRequest.send(null);
}
...
{
	//alert("from HO_CO recovery");
	var empNo=document.getElementById("empNo").value;	
	var url='penAppHOForm1CalcFetchOldRecdata.html?pensionAppCalcHOForm1.empNo='+empNo+"&rid="+Math.random();
...
function fetchCommutedvalueData()
{
	...
	//alert(age);
		var url="compenAppHOForm1CalcFetchMstdata.html?pensionAppCalcHOForm1.age="+age;	 
...
function fetchCommutedvalueData()
{
	...
	/*  dob="05/11/1953";//58
	retirementdate="03/11/2011";*/
	
	/*dob="05/11/1953";//59
	retirementdate="06/11/2011";*/
	
	/*dob="31/12/1953";//59
	retirementdate="31/12/2011";*/
	...
	//alert("TOYEAR"+d2.getFullYear()+"TOMONTH"+d2.getMonth()+"TODAYS"+d2.getDate());
	//alert("FROMYEAR"+d1.getFullYear()+"FROMMONTH"+d1.getMonth()+"FROMDAYS"+d1.getDate());
	...
	//alert("fetchCommutedvalueData"+age);
		var url="compenAppHOForm1CalcFetchMstdata.html?pensionAppCalcHOForm1.age="+age;	 
		var comreq=getTransportWth();	    
		comreq.open("GET",url,true);        
		comreq.onreadystatechange=function()
...
function fetchEmployeeStatusData()
{
	...
	{
		var url="empStatusAppHOForm1CalcFetchMstdata.html?pensionAppCalcHOForm1.classPensionId="+typepension+"&pensionAppCalcHOForm1.empNo="+empNo;	 
	 ...
	 if(empstatreq.readyState==4)
	  {
	      ...
	      {  
	    	  ...
	    	  var empstatbaseResponse=empstatreq.responseXML.getElementsByTagName("empstatresponse")[0];	    	  	    	  
	    	  ...
	    	  { 	    		  
	    		  ...
	    		  // else if(employeestatus.toLowerCase()=="wkg")
	    			  else if((employeestatus.toLowerCase()!="vrs") && (employeestatus.toLowerCase()!="dth"))
		    		  {
...
{	
	...
	{	
		...
		calcworkservice();
		calcworkservice_dupli();
		//alert("Count in other defaultselection:"+count++);
		calctotalnqs();
		*/
		calcservice();
	}
...
function typeofpension_selection()
{
	...
	var typeofpensionid=document.getElementById("classPensionId");
	var selectedindex=typeofpensionid.selectedIndex;
	//var selectedindex=parseInt(typeofpensionid);
	//alert("selectedindex ::: "+selectedindex);
	var selectedtext=typeofpensionid.options[selectedindex].text;
	//alert("Inside type of pension  ::"+selectedtext);
	if(selectedtext=="Super Annuation")
	{
		...
		//alert("Before if stat");
		if(document.getElementById("empNo").value!="")
		{
			//alert("Before Call Service");
			calcservice();
			//calcworkservice();
			//calcworkservice_dupli();
...
//alert("eligible_pension_validation")
	...
	if((penvalflag==1) ||(lastvalflag==1) || (aevalflag==1) || (recvalflag==1) || (eligiblevalflag==1))
	{
		//calcservice();
		return false;		
	}
	...
	var level=document.getElementById("lastlevel").value;
	
	if(!level)
		{
		alert("Enter Level of Pay");
		return false;
		}
	...
	*/
	else
	{
		 ...
		 //alert("Pension amount"+pensionamount);
		  ...
		  if((document.getElementById("pensionpertFlagpert").checked==true) && (document.getElementById("commPert").value!="") && (document.getElementById("pensionCommutedFlagYes").checked==true))
		  {
			  ...
			  //alert("The pension Amount is"+pensionamount);
			  //alert("The pension compert is"+compert);
			  if(Math.round(pensionamount) < minpensionamount)
			  {
				  pensionamount=minpensionamount;			  
			  }
			  commutedamount=Math.floor(((parseInt(pensionamount)*parseFloat(compert/100))));
		  ...
		  {
			  //alert("else The pension Amount is"+pensionamount);
			  if(Math.round(pensionamount) < minpensionamount)
			  {
				  pensionamount=minpensionamount;			  
			  }
			 
			  commutedamount=Math.floor((parseFloat((pensionamount)/3)));
			 //alert(" else commutedamount"+commutedamount);
		  }
		  ...
		  if(typeofpension!="" && selectedtext=="Death")
			{			    
				...
				fampesnionamount=(lasttotal)*parseFloat((fampensionamountbeforceiling)/100);
				fampensionamountafter=(lasttotal)*parseFloat((fampensionamountafterceiling)/100);
		  ...
		  //alert("Reduced commuted amount"+reducedpensionamount);
...
function ae_form_final_validation()
{
	try
	{
			...
			for(i=0; i<document.AEForm.todate.length; i++)
			{
				...
				var datesSep1=ff.split("/");
					...
					else
					{
						...
						if(totday>0)
						{
							if(datesSep[0]==31 && fromtotday==30 && datesSep1[0]!=1)
							{
								totday=totday;			
							}
							else
		...
		//	var lastgradepay=0;
			...
			{
			var =parseInt(document.getElementById("lastgrade").value);
			}*/
			...
			if(  (finalspecialpay!=lastspecialpay) || (finaloptionpay1!=lastother1pay) || (finaloptionpay2!=lastother2pay) || (finaloptionpay3!=lastother3pay))
			{
...
//	calcworkservice();
//	calcworkservice_dupli();
	//alert("Count in other cleanNON_QUAL_SER:"+count++);
	//calctotalnqs();	
}
