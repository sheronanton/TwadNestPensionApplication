...
<title>Search Pension Application Form1 (Calculation) For Validate -Head Office</title>
<link rel=StyleSheet href="<s:url value="/css/Sample3.css" />"
	type="text/css">
	
<script
	src="<s:url value="/javascript/PensionApplicationHeadOfficeForm1ValSearch.js" />"
...
function post_valueFamSrch(){
	...
	document.getElementById("penAppHOForm1empNo").value=emp_no_td_value;

	opener.document.AEForm.empNo.value=emp_no_td_value;	
	opener.fetchMstData(emp_no_td_value);	
	window.close();
...
<s:form action=""	name="" namespace="/pages">
	<table width="70%" align="center" border="1">
		<tr>
			<td colspan="4" background="../images/bluebg1.jpg" align="center"><font	color="white" size="3"><strong>Search Pension Application Form1 (Calculation) For Validate -Head Office</strong></font></td>
		</tr>
		<tr>
			<td align="center" height="30" valign="middle">
			<s:select headerKey=""
				headerValue="-- Select --" id="SearchScreenOptions" name="penAppHeadOfficeform1ValSrch.SearchScreenOptions"
				list="#{'2':'Emp No','3':'Employee Name'}"
				theme="simple" cssStyle="border:#ffba14 1.5px solid"  onchange="javascript:cleanTbodyFai()" />&nbsp;&nbsp;
			<s:textfield  id="SearchScreenKeyword" name="penAppHeadOfficeform1ValSrch.SearchScreenKeyword" theme="simple"
				cssStyle="border:#ffba14 1.5px solid" />&nbsp;&nbsp; 
			<input type="button" id="Search" name="Search" Value="Search"
				onclick="javascript:penAppHeadOfficeForm1Search('Search')" />&nbsp;&nbsp;
			<input type="reset" id="btnClear" name="Clear" value="Clear" onclick="javascript:cleanTbodyFai()" />
			<s:hidden name="" id="penAppHOForm1empNo" theme="simple">
			</s:hidden>
	...
	<br>
	<center>
<!--	<input type="submit" name="Submit" id="Submit" value="Submit" onclick="post_valueFamSrch();" /> -->
